...
## 설치
### 1. [git-scm](https://git-scm.com) 에 접속하여 우측 중앙의 다운로드 번튼을 클릭한다.
...
### 5. 회원가입 후 발송도니 이메일에서 인증을 하고 로그인 한다.
### 6. 다시 터미널창에 접속하여 아래의 명령을 실행한다.
```bash
git config --global user.name 'YUNJINI'
git config --global user.email 'ekal2001@naver.com'
```
